...
func WriteBytes(conn net.Conn, content []byte) (int, error) {
	writer := bufio.NewWriter(conn)
...
func (self *AsyncTcpSender) SendBytes(content []byte) (int,error) {
	self.lock.Lock()
	defer self.lock.Unlock()
	if !self.active {
		return 0,errors.New("Send Error: Uninitialized sender!")
	}
	return WriteBytes(self.conn, content)
}
